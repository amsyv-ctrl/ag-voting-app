...
update votes v
set vote_round = b.vote_round
from ballots b
where v.ballot_id = b.id
  and (v.vote_round is null or v.vote_round < 1);

create index if not exists idx_votes_ballot_round on votes(ballot_id, vote_round);
...
declare
  ...
  v_choices jsonb;
begin
  select b.id, b.slug, b.title, b.description, b.ballot_type, b.majority_rule, b.status, b.opens_at, b.closes_at, b.vote_round, e.name as event_name
  into v_ballot
  from ballots b
...
declare
  ...
  v_rows jsonb;
begin
  select b.id, b.majority_rule, b.status, b.closes_at, b.vote_round
  into v_ballot
  from ballots b
  where b.slug = p_slug
  limit 1;
...
begin
  ...
  if exists (
    ...
    raise exception 'PIN already used in this event' using errcode = 'P0001';
  end if;
